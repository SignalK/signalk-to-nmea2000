...
module.exports = (app, plugin) => {
  return [{
    ...
    conversions: (options) => {
      sourcedata = options?.RAYMARINENAV?.sourcedata || "Course API v2"
      return [{
        pgn: 130848,
        keys: raymarine130848Keys[sourcedata],
        sourceType: 'timer',
        interval: 1000,
        callback: async (app, distance, bearingTrue, bearingMagnetic) => {
          ...
          waypointId = (course.activeRoute?.pointIndex || 0) + 1
          return [{
            pgn: 130848,
            "dst": 255,
...
// "Unknown": 4, penultimate stop
// "Unknown": 13, last stop
// "Distance, Position to Next Waypoint" is to Current Waypoint when Unknown = 13
// "Bearing, Position to Next Waypoint, True" is to Current Waypoint when Unknown = 13
// "Bearing, Current Waypoint to Next Waypoint, True" = 0.0
      {
        pgn: 130918,
        keys: raymarine130918Keys[sourcedata],
        sourceType: 'timer',
        interval: 1000,
        callback: async (app, position, distance, bearingTrue) => {
          ...
          unknownFlags = 0
          bearingCurrentToNext = undefined
          bearingPositionToNext = undefined
          distancePositionToNext = undefined
          ...
          if (course.activeRoute == null)
          {
            ...
            currentWaypointName = "Waypoint " + waypointId.toString()
          }
